...
export class News extends Component {
  ...
  async componentDidMount(){
    ...
    let data= await fetch(url)
    let parsedData=await data.json()
    console.log('compoDiMount',parsedData)
    this.setState({articles:parsedData.articles,
      totalResults:parsedData.totalResults,
      loading:false,
      page:2})
  ...
  // }
    someFxn= async()=>{
      ...
      let data= await fetch(url);
      let parsedData=await data.json();
      console.log('just got it from infiScroll',parsedData);
  ...
  render() {
    return (
      <>
        <h2 className='text-center'>STONKS-NEWS - TOP {this.capitalise(this.props.category)} HEADLINE</h2>
